...
public class Kelompok10Application {
	...
	CommandLineRunner runner(UserService userService, RoleService roleService){
		return args -> {
			...
			userService.createUser(new UserEntity(1L, "Manager", "manager", "buatpasswordyangsusah", "manager@email.com", 123456L, null, null, new ArrayList<>()));
			userService.createUser(new UserEntity(2L, "Admin", "admin", "yanginijugasusah", "admin@email.com", 123456L, null, null, new ArrayList<>()));
			userService.createUser(new UserEntity(3L, "Rakha", "rakha", "yanginigampang", "user@email.com", 123456L, null, null, new ArrayList<>()));
